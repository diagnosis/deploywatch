...
func (h *GitHubHandler) HandleListRepos(w http.ResponseWriter, r *http.Request) {
	...
	if len(installations) == 0 {
		responder.JSON(w, http.StatusOK, []any{}, correlationID)
		return
	}

	token, err := h.gitHubClient.GetInstallationToken(ctx, installations[0].InstallationID)
	if err != nil {
		logger.Error(ctx, "failed to get installation token", "err", err)
		responder.Error(w, err, correlationID)
		return
	}

	repos, err := h.gitHubClient.ListInstallationRepos(ctx, token, installations[0].InstallationID)
	if err != nil {
		logger.Error(ctx, "failed to list repos", "err", err)
		responder.Error(w, err, correlationID)
		return
	}

	responder.JSON(w, http.StatusOK, repos, correlationID)
}
